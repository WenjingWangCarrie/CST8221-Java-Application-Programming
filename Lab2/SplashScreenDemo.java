...
public class SplashScreenDemo extends JWindow {
	...
	public void showSplashWindow() {
		...
		// Set the window's bounds, position the window in the center of the
		// screen
		int width = 534 + 10;
		int height = 263 + 10;
		...
		// accessing an image in a package named splash without using
		// getResource - needs an absolute path
		...
		// ImageIcon(getClass().getResource("/splash/SplashSwing.gif")));

		// image placed in folder named resources placed into the source folder
		// JLabel label = new JLabel(new
		// ImageIcon(getClass().getResource("/splash/resources/SplashSwing.gif")));

		// using URL with an image placed in resources folder in source folder
		// URL imgURL =
		// this.getClass().getResource("resources/SplashSwing.gif");
		// Image img = Toolkit.getDefaultToolkit().getImage(imgURL);
		// JLabel label = new JLabel(new ImageIcon(img));
		// accessing an image in the default packge
		// works if compiled and run from command prompt - not if run from
		// Netbeans
	 ...
	 * 
	 * @param args
	 *            the time duration of the splash screen in milliseconds. If not
	...
	public static void main(String[] args) {
		...
		// Normally,the splash.showSplashWindow() will be called by the main
		// application class.
