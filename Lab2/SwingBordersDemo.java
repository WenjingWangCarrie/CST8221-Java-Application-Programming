 ...
 * JLabel is used for the demonstration, but a border can be painted around
 * every JComponent object.
 * 
 * @version 1.17.2
...
public class SwingBordersDemo extends JFrame {
	...
	private void createGUI() {
		...
		line = BorderFactory.createLineBorder(Color.red); // create Rounded Line
															// Border
		...
		Border[] borders = { empty, tickEmptyBoarder, line, thickLine, raisedbevel, loweredbevel, raisedEtched,
		...
		String[] labelText;
		labelText = new String[] { "empty boarder", "tick empty boarder", "line boarder", "thick rounded line boarder",
				"raised bevel boarder", "lowered bevel boarder", "raised etched boarder", "lowered etched boarder",
				"default titled boarder", "modified titled boarder", "compound border(raised bevel and line)",
	...
	public static void main(String[] args) {
		/*
		 * Make all components to configured by the event dispatch thread. This
		 * is the thread that passes user provoked events such as mouse clicks
		 * to the GUI components which have registered listeners for the events.
		 * The following code fragment forces the statements in the run() method
		 * to be executed in the event dispatch thread. The
		 * SwingUtilities.invokeLater() actually calls EventQueue.invokeLater()
		 * method.
		 */
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				// no need to use explicitly a WindowListener. The line below
