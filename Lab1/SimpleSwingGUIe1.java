...
public class SimpleSwingGUIe1 extends WindowAdapter implements ActionListener {
	...
	private void setAndLaunch() {
		...
		// !!! (Important in Assignment 1) 
		// Create a keyboard shortcut(mnemonic): Pressing ALT-I will act as a
		// mouse click on the button. 
		button.setMnemonic('i');
		// !!! Create a Tool Tip. Will show up when the mouse hovers over the
		// button.
		button.setToolTipText("Please Click Me");
		
		// Register an event handler to handle the events generated by the user
		// actions affecting the button(for example, mouse click)
		// The application class serves as an event handler - that is why this
		// reference is used.
		...
		/*
		 * Arrange the components and the containers inside the primary
		 * container.
		 */
		...
		// If you use this alternative you should make sure that the repacment
		...
		// The location of the frame on the screen will by determined by the
		// operating system
	...
	/**
	 * Handles the frame closing event Set up the Close button (X) of the frame.
	 * You should use this method only if you need to free resources or close
	 * connections or files before the application closes. Override the
	 * windowClosing() method of Window Adapter
	 * 
	 * @param e
	 *            window event supplied by the runtime
	 */
	@Override
	 ...
	 * 
	 * @param e
	 *            action event supplied by the runtime
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		// Check which button was clicked and process the event
		Object eventSource = e.getSource();
		if (eventSource == button) { // they are the same object
		//	eventSource. //shows the object, because it only has reference to object
		//	button.      // shows the container and component. 
			
			// !!! if you want to cast, need to use 'instanceof'
			if (eventSource instanceof JButton) {
				// cannot put different data type into array list, unless you cast
			}
			String s = ((JButton) eventSource).getText(); 
