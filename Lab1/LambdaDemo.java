/**
 * This class demonstrates the basics of Java 8 Lambda Expressions. A Lambda
 * Expression is, essentially, an anonymous (unnamed) method. Since it is
 * anonymous it cannot be executed on its own. Instead, the lambda expression is
 * used to implement a method defined by a target functional interface. Thus, a
 * lambda expression results in a form of anonymous class. Lambda expressions
 * are also referred as closures.
 * 
 * A Functional Interface is an interface that contains one and only one
 * abstract method (since Java 8 an interface can also have default methods).
 * Thus, a functional interface typically represents a single action. A
 * functional interface can be used as a target type of a lambda expression. A
 * lambda expression can be used only in a context in which a target type is
 * specified. A Functional Interfaces is also known as a SAM type, where SAM
