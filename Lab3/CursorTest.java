 ...
 * 
 * @version 1.17.2
 * @author Svillen Ranev #since Java 1.2
 */
public class CursorTest extends JFrame {
	// default serial version ID - Swing components implement the Serializable
	// interface
	// generate a real serial version ID after your code has been completed and
	// working
	...
	public CursorTest() {
		...
		// initialize the array of cursors here using the predefined constants
		// in the Cursor class use an anonymous array to perform the
		// initialization
		/*******************************/
		cursors = new int[] { DEFAULT_CURSOR, CROSSHAIR_CURSOR, TEXT_CURSOR, WAIT_CURSOR, N_RESIZE_CURSOR,
				S_RESIZE_CURSOR, W_RESIZE_CURSOR, E_RESIZE_CURSOR, SW_RESIZE_CURSOR, SE_RESIZE_CURSOR, NW_RESIZE_CURSOR,
				NE_RESIZE_CURSOR, HAND_CURSOR, MOVE_CURSOR };
		/*******************************/
	...
	/**
	 * Creates a button and registers (adds) an ActionListener to process the
	...
	private JButton createButton(String buttonName) {
		...
		// in most cases it is better to leave to the UI Manager to determine
		// the preferred size
		button.setPreferredSize(new Dimension(button.getText().length() * 20, 50));
		// Create a keyboard shortcut: Pressing ALT-C will act as a mouse click
		// on the button
		button.setMnemonic('C');
		// Create a Tool Tip. Will show up when the mouse hovers over the button
		button.setToolTipText("Please Click Me");

		// Handle the button clicks. Anonymous class is used to handle the
		// events.
		// No need to check for the event source anymore.
		// Now each individual button will be responsible to handle its own
		// events.
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				if (eventSource == button) {
					...
					button.setCursor(Cursor.getPredefinedCursor(cursors[cursorCounter]));

					// set the label text here: use getName to get the name of the cursor
					label_02.setText(LABEL_TEXT + Cursor.getPredefinedCursor(cursors[cursorCounter]).getName());
				}
	 ...
	 * Anonymous class is used to pass the run method to the event dispatching
	 * thread.
	 * 
	 * @param args
	 *            not used
